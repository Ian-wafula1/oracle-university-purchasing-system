	...
	return (
		<>
			...
			<div className="page-container space-y-6">
				<PageHeader title="Invoice Totals Report" breadcrumbs={['Reports', 'Invoice Totals']} />
					...
					<table className="w-full text-sm">
						...
						<tbody>
							{isLoading ? (
								...
								filteredRows.map((r) => {
									const rowBg = r.totaldisputed > 0 ? 'bg-destructive/5' : r.totaloutstanding > 0 ? 'bg-amber-500/5' : '';

									return (
										<tr key={`${r.supplierid}-${r.invoiceyear}-${r.invoicemonth}`} className={`border-b transition-colors hover:bg-muted/40 ${rowBg}`}>
											...
											<td className="px-4 py-3 text-right text-emerald-600 font-medium">{formatCurrency(r.totalpaid)}</td>
											<td className="px-4 py-3 text-right">{r.totaloutstanding > 0 ? <span className="text-amber-600 font-medium">{formatCurrency(r.totaloutstanding)}</span> : <span className="text-muted-foreground">—</span>}</td>
											<td className="px-4 py-3 text-right">{r.totaldisputed > 0 ? <span className="text-destructive font-medium">{formatCurrency(r.totaldisputed)}</span> : <span className="text-muted-foreground">—</span>}</td>
						...
						{!isLoading && filteredRows.length > 0 && (
							<tfoot>
								<tr className="border-t-2 bg-muted/50 font-semibold">
									...
									<td className="px-4 py-3 text-right text-amber-600">{totals.outstanding > 0 ? formatCurrency(totals.outstanding) : '—'}</td>
									<td className="px-4 py-3 text-right text-destructive">{totals.disputed > 0 ? formatCurrency(totals.disputed) : '—'}</td>
									<td />
								</tr>
							</tfoot>
						)}
